...
fork(int tickets)
{
  ...
  *np->tf = *curproc->tf;
  ...
  if(tickets > 500) tickets = 500;
  else if(tickets < 10) tickets = 10;

  init_proc(np, tickets);
  np->exec_times = 0;
...
procdump(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    ...
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    cprintf("%d %s %s tckts: %d times: %d stride: %d pass: %d", p->pid, state, p->name, p->tickets, p->exec_times, p->stride, p->pass);
    if(p->state == SLEEPING){
