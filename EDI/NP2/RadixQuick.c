...
int partitionVetor(TpContato vetor[], int p, int q) {
  ...
  while(i < j){
    while(strcmp(pivo.nome, vetor[i].nome) >= 0) i++;
    while(strcmp(pivo.nome, vetor[j].nome) <  0) j--;
    if(i < j){
      aux = vetor[i];
      vetor[i] = vetor[j];
      vetor[j] = aux;
...
void trocaContatoLista(TpContato *i, TpContato *j){
  TpContato aux;

  strcpy(aux.nome, j->nome);
  strcpy(aux.fone, j->fone);

  strcpy(j->nome, i->nome);
  strcpy(j->fone, i->fone);

  strcpy(i->nome, aux.nome);
  strcpy(i->fone, aux.fone);
}
...
TpContato * partitionLista(TpContato *p, TpContato *q) {
  ...
  while(i != j){
    ...
    if(i != j){
      trocaContatoLista(i, j);
      j = j->ant;
    }
  }

  trocaContatoLista(pivo, j);
